...
# Initialize FastAPI App (Top-Level 'app' exported for Vercel Serverless Runtime)
app = FastAPI(
    ...
    <script>
        ...
        async function handleQuery(e) {{
            ...
            try {{
                ...
                outBox.innerHTML = `<h3>Analysis Report [Category: ${{(data.task || currentMode).toUpperCase()}}]</h3>\n<p>${{data.answer}}</p>\n\n<p style="color: #CCFF00; margin-top: 1rem;"><b>Confidence:</b> ${(data.confidence * 100).toFixed(1)}% | <b>Execution Time:</b> ${{data.execution_time_sec}}s</p>`;
...
def get_dashboard():
    """Serves the SatQuery AI Vercel Web Dashboard."""
    return HTML_TEMPLATE

@app.post("/api/query")
async def process_satquery_api(
    ...
    """
    API Endpoint for processing Remote Sensing VQA, Captioning, and Change Detection queries.
    """
    images = []
    if image1 is not None:
        content = await image1.read()
        try:
            img1 = Image.open(io.BytesIO(content)).convert("RGB")
            images.append(img1)
        except Exception:
            pass
    ...
    if image2 is not None:
        ...
        try:
            img2 = Image.open(io.BytesIO(content)).convert("RGB")
            images.append(img2)
        except Exception:
            pass

    # Default fallback sample image if no image uploaded
    if not images:
        default_img = Image.new("RGB", (256, 256), color=(34, 139, 34))
        images.append(default_img)

    is_change_mode = (task_mode == "change" or len(images) >= 2)
    result = orchestrator.execute_query(query=query, images=images, is_optical_sar_pair=False)
